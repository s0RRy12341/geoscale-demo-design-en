  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: Math.round(size * 0.26), fontWeight: 500, color: theme.text }}>{percent}%</span>
      </div>
    </div>
  );
}
    ...
    <div ref={containerRef} style={{ position: "relative", width: "100%" }}>
      <svg width={chartW} height={chartH} viewBox={`0 0 ${chartW} ${chartH}`} style={{ display: "block" }}>
        {gridValues.map((v, i) => (
          <g key={i}>
            ...
            <text x={padLeft - 8} y={getY(v) + 4} textAnchor="end" fill={theme.textMuted} fontSize="13" fontFamily="Inter, sans-serif">{Math.round(v)}%</text>
          </g>
        ))}
        <polygon points={geminiAreaPoints} fill="#727272" opacity="0.06" />
        <polygon points={gptAreaPoints} fill="#10A37F" opacity="0.08" />
        <polyline points={geminiPoints} fill="none" stroke="#727272" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
        ...
        {data.gemini.map((v, i) => (
          <circle key={`gem-${i}`} cx={getX(i)} cy={getY(v)} r={hoverIdx === i ? 6 : 3.5} fill="#727272" stroke={theme.cardBg} strokeWidth={hoverIdx === i ? 2 : 0} style={{ transition: "r 150ms" }} />
        ))}
        {data.labels.map((label, i) => (
          <text key={i} x={getX(i)} y={chartH - 5} textAnchor="middle" fill={theme.textMuted} fontSize="13" fontFamily="Inter, sans-serif">{label}</text>
        ))}
        {data.labels.map((_, i) => (
      ...
      {hoverIdx !== null && (
        <div style={{
          ...
          <div style={{ fontWeight: 500, marginBottom: 4 }}>{data.labels[hoverIdx]}</div>
    ...
    <div ref={containerRef} style={{ position: "relative", width: "100%" }}>
      <svg width={chartW} height={chartH} viewBox={`0 0 ${chartW} ${chartH}`} style={{ display: "block" }}>
        {gridValues.map((v, i) => (
          <g key={i}>
            <line x1={padLeft} y1={getY(v)} x2={chartW - padRight} y2={getY(v)} stroke={theme.border} strokeWidth="1" />
            <text x={padLeft - 8} y={getY(v) + 4} textAnchor="end" fill={theme.textMuted} fontSize="13" fontFamily="Inter, sans-serif">{Math.round(v).toLocaleString()}</text>
          </g>
        ))}
        <defs>
...
function PersonaBadge({ personaId, theme }: { personaId: string; theme: Theme }) {
  ...
  if (!p) return null;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 15, fontWeight: 500, padding: "2px 8px", borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.cardBg, color: theme.text }}>
      {p.name}
    </span>
  );
}
...
function StageBadge({ stage, theme }: { stage: string; theme: Theme }) {
  return (
    <span style={{ display: "inline-flex", fontSize: 15, fontWeight: 500, padding: "2px 8px", borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.badgeBg, color: theme.text }}>
      {stage}
    </span>
  );
}
...
function MentionBadge({ mentioned }: { mentioned: boolean }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 15, fontWeight: 500, padding: "2px 8px", borderRadius: 10, background: mentioned ? "transparent" : "transparent", color: mentioned ? "#10A37F" : "#727272", border: `1px solid ${mentioned ? "#10A37F" : "#727272"}` }}>
...
export default function ScanPage() {
  ...
  const sectionTitle: React.CSSProperties = { fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 };
          ...
          <div style={{ display: "flex", gap: 0, overflowX: isMobile ? "auto" : "visible", WebkitOverflowScrolling: "touch" as any, scrollbarWidth: "none" as any, msOverflowStyle: "none" as any }}>
            {([
              ...
              >
                ...
                {tab.count !== undefined && (
                  <span style={{ fontSize: 15, padding: "1px 6px", borderRadius: 10, background: theme.badgeBg, color: theme.textSecondary, border: `1px solid ${theme.border}` }}>{tab.count}</span>
                )}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: isMobile ? "12px 10px" : "20px 24px", flex: 1, width: "100%", minWidth: 0, boxSizing: "border-box" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {[
                ...
                <div key={i} style={{ ...card, padding: "12px 14px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                    <span style={{ fontSize: 28, fontWeight: 500, color: theme.text, letterSpacing: "-1px" }}>{stat.value}</span>
            ...
            <div style={{ ...card, padding: 0, overflow: "hidden" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(6, 1fr)" }}>
                {[
                  { label: "Domain Rating", value: "42", icon: "DR", color: "#10A37F", tooltip: "Ahrefs Domain Rating (0-100). Measures the strength of the website's backlink profile relative to other sites.", href: null },
                  { label: "Organic Traffic", value: "8,240", icon: "OT", color: "#10A37F", tooltip: "Estimated monthly organic visitors from Google search. Source: Google Search Console + Ahrefs estimate.", href: "/traffic-detail" },
                  { label: "Ranking Keywords", value: "347", icon: "KW", color: theme.text, tooltip: "Total keywords this domain ranks for in Google's top 100 results. Source: Ahrefs keyword tracking.", href: "/keywords-detail" },
                  { label: "Backlinks", value: "1,892", icon: "BL", color: theme.text, tooltip: "Total external backlinks pointing to this domain from other websites. Source: Ahrefs backlink index.", href: "/backlinks-detail" },
                  { label: "Ref. Domains", value: "156", icon: "RD", color: theme.text, tooltip: "Number of unique referring domains linking to this site. More diverse domains = stronger authority signal.", href: null },
                  { label: "Indexed Pages", value: "89", icon: "IX", color: theme.text, tooltip: "Pages from this domain currently indexed in Google. Source: Google Search Console index coverage report.", href: null },
                ].map((m, i) => {
                  const inner = (
                    <div key={i} style={{ padding: isMobile ? "10px 10px" : "12px 14px", borderRight: isMobile ? (i % 2 === 0 ? `1px solid ${theme.border}` : "none") : (i < 5 ? `1px solid ${theme.border}` : "none"), borderBottom: isMobile && i < 4 ? `1px solid ${theme.border}` : "none", position: "relative", cursor: m.href ? "pointer" : "default", transition: "background 150ms" }}
                      onMouseEnter={(e) => { if (m.href) (e.currentTarget as HTMLDivElement).style.background = theme.hoverBg; }}
                      onMouseLeave={(e) => { if (m.href) (e.currentTarget as HTMLDivElement).style.background = "transparent"; }}
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
                        <span style={{ fontSize: 20, fontWeight: 500, color: m.color, letterSpacing: "-0.5px" }}>{m.value}</span>
                        {m.href && (
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={theme.textMuted} strokeWidth="2" style={{ flexShrink: 0 }}>
                  ...
                  return m.href ? (
                    <a key={i} href={m.href} style={{ textDecoration: "none", color: "inherit", display: "contents" }}>{inner}</a>
                  ) : inner;
                })}
              </div>
            </div>
            ...
            <div style={{ ...card, padding: isMobile ? 12 : 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>Mention rate over time</h3>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
                  {(["7", "30", "90"] as const).map((p) => (
                    <HoverButton key={p} onClick={() => setChartPeriod(p)} theme={theme} style={{ padding: "6px 14px", fontSize: 15, fontWeight: chartPeriod === p ? 600 : 500, ...(chartPeriod === p ? btnActive : btnInactive), borderRadius: p === "7" ? "9px 0 0 9px" : p === "90" ? "0 9px 9px 0" : "0", cursor: "pointer", marginLeft: p !== "7" ? -1 : 0 }} filled={chartPeriod === p}>
                      {p} days
                    </HoverButton>
                  ))}
                </div>
              ...
              {JOURNEY_STAGES.map((stage, i) => {
                ...
                return (
                  <div key={i} style={{ ...card, padding: 14, textAlign: "center" }}>
                    <div style={{ fontSize: 28, fontWeight: 500, color: stage.percent >= 80 ? "#10A37F" : theme.text, marginBottom: 2 }}>{stage.percent}%</div>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, fontSize: 15, fontWeight: 500, color: theme.text, marginBottom: 2 }}>
                      {stage.name}
                      <Tooltip text={journeyTooltips[stage.name] || "Customer journey stage"} />
                    </div>
                    <div style={{ fontSize: 15, color: theme.textSecondary }}>{stage.count} queries</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ ...card, padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>Top 5 Keywords</h3>
              ...
              <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" as any }}>
                <table style={{ width: "100%", fontSize: 15, borderCollapse: "collapse", minWidth: isMobile ? 600 : "auto" }}>
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${theme.border}` }}>
                      <th style={{ textAlign: "left", padding: "7px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Keyword</th>
                      <th style={{ textAlign: "right", padding: "7px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Volume</th>
                      <th style={{ textAlign: "left", padding: "7px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>KD</th>
                      <th style={{ textAlign: "left", padding: "7px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Position</th>
                      <th style={{ textAlign: "left", padding: "7px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Matching AI Queries</th>
                    </tr>
                  </thead>
                  <tbody>
                    {KEYWORD_DATA.map((kw, i) => (
                      <tr key={i} style={{ borderBottom: thinBorder }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = theme.hoverBg; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = theme.cardBg; }}>
                        <td style={{ padding: "8px 10px", fontWeight: 400, color: theme.text }}>{kw.keyword}</td>
                        <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 400, color: theme.text }}>{kw.volume.toLocaleString()}</td>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                            {kw.matchingQueries.map((q, j) => (
                              <span key={j} style={{ display: "inline-flex", fontSize: 15, padding: "2px 8px", borderRadius: 10, border: `1px solid ${theme.border}`, background: theme.badgeBg, color: theme.text, maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{q}</span>
                            ))}
                          </div>
                        </td>
                      </tr>
            ...
            <div style={{ ...card, padding: isMobile ? 12 : 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>SEO Performance</h3>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 15, color: theme.text }}>Organic Traffic</span>
                  </div>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 15, fontWeight: 500, padding: "3px 10px", borderRadius: 10, background: theme.badgeBg, color: "#727272", border: `1px solid ${theme.border}` }}>
            ...
            <div style={{ ...card, padding: isMobile ? 12 : 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>SEO {"<>"} GEO Connection</h3>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>SEO</span>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>GEO</span>
                    ...
                    <div key={i} style={{ padding: "14px 0", borderBottom: i < KEYWORD_DATA.length - 1 ? `1px solid ${theme.border}` : "none" }}>
                      {seoToggle && (
                        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: geoToggle ? 8 : 0, flexWrap: "wrap" }}>
                          ...
                          <span style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>{kw.keyword}</span>
            ...
            <div style={{ ...card, padding: 16 }}>
              ...
              <table style={{ width: "100%", fontSize: 15, borderCollapse: "collapse", minWidth: isMobile ? 550 : "auto" }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${theme.border}` }}>
                    <th style={{ textAlign: "left", padding: "8px 12px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Competitor</th>
                    <th style={{ textAlign: "left", padding: "8px 12px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>GEO Score</th>
                    <th style={{ textAlign: "left", padding: "8px 12px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Query overlap</th>
                    <th style={{ textAlign: "center", padding: "8px 12px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><AIEngineLogo engine="gpt" size={14} /> <AIEngineLogo engine="gemini" size={14} /></span></th>
                    <th style={{ textAlign: "left", padding: "8px 12px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>Trend</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderBottom: thinBorder }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = theme.hoverBg; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = theme.cardBg; }}>
                      <td style={{ padding: "10px 12px" }}>
                        <div>
                          ...
                          <span style={{ display: "block", fontSize: 15, color: theme.textMuted }}>{c.domain}</span>
                        </div>
                      </td>
                      <td style={{ padding: "10px 12px" }}><span style={{ fontSize: 15, fontWeight: 500, color: c.geo >= 60 ? "#10A37F" : theme.text }}>{c.geo}%</span></td>
                      <td style={{ padding: "10px 12px" }}><span style={{ fontSize: 15, color: theme.text }}>{c.overlap} queries</span></td>
            ...
            <div style={{ ...card, padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>Top 5 queries</h3>
              ...
              <table style={{ width: "100%", fontSize: 15, borderCollapse: "collapse", minWidth: isMobile ? 550 : "auto" }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${theme.border}` }}>
                    <th style={{ textAlign: "left", padding: "8px 10px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Query <Tooltip text="The query tested against AI engines" /></span></th>
                ...
                <tbody>
                  {TOP_5_QUERIES.map((q) => (
                    <tr key={q.id} style={{ borderBottom: thinBorder }}>
                      <td style={{ padding: "10px 10px", fontWeight: 400, color: theme.text }}>{q.text}</td>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
              <div style={{ ...card, padding: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>What worked</h3>
            ...
            <div style={{ ...card, padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>AI Summary - What engines say about you</h3>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
                <div style={{ border: thinBorder, borderRadius: 10, padding: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 500, color: "#10A37F" }}>ChatGPT Summary</span>
                  </div>
                ...
                <div style={{ border: thinBorder, borderRadius: 10, padding: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 500, color: "#727272" }}>Gemini Summary</span>
                  </div>
                  <p style={{ fontSize: 15, lineHeight: 1.7, color: theme.text, margin: 0 }}>
            ...
            <div style={{ ...card, padding: 16, borderLeft: "4px solid #10A37F" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: 0 }}>AI Insights</h3>
          ...
          <div>
            <div style={{ ...card, padding: 14, marginBottom: 14 }}>
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  <HoverButton onClick={() => setPersonaFilter("all")} filled={personaFilter === "all"} theme={theme} style={{ padding: "5px 12px", borderRadius: 9, fontSize: 15, fontWeight: personaFilter === "all" ? 600 : 500, ...(personaFilter === "all" ? btnActive : btnInactive), cursor: "pointer" }}>
                    All personas
                  </HoverButton>
                  {PERSONAS.map((p) => (
                    <HoverButton key={p.id} onClick={() => setPersonaFilter(p.id)} filled={personaFilter === p.id} theme={theme} style={{ padding: "5px 12px", borderRadius: 9, fontSize: 15, fontWeight: personaFilter === p.id ? 600 : 500, ...(personaFilter === p.id ? btnActive : btnInactive), cursor: "pointer" }}>
                      {p.name} - {p.role}
                    </HoverButton>
                  ))}
                </div>
              ...
              <table style={{ width: "100%", fontSize: 15, borderCollapse: "collapse", minWidth: isMobile ? 650 : "auto" }}>
                <thead>
                  <tr style={{ background: theme.tableHeaderBg, borderBottom: `1px solid ${theme.border}` }}>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>#</th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Query <Tooltip text="The query tested against AI engines" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Persona <Tooltip text="The target-audience profile this query belongs to" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Journey stage <Tooltip text="Customer journey stage: Awareness, Research, Decision, Support" /></span></th>
                    <th style={{ textAlign: "center", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><AIEngineLogo engine="gpt" size={16} /> <AIEngineLogo engine="gemini" size={16} /> <AIEngineLogo engine="perplexity" size={16} /></span></th>
                  ...
                  {filteredQueries.map((q) => (
                    <React.Fragment key={q.id}>
                      <tr onClick={() => setExpandedQuery(expandedQuery === q.id ? null : q.id)} style={{ borderBottom: expandedQuery === q.id ? "none" : thinBorder, cursor: "pointer", transition: "all 150ms", background: theme.tableBg }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = theme.hoverBg; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = theme.tableBg; }}>
                        <td style={{ padding: "10px 14px", fontWeight: 400, color: theme.textMuted }}>{q.id}</td>
                        <td style={{ padding: "10px 14px", fontWeight: 400, color: theme.text, maxWidth: 320 }}>{q.text}</td>
                            ...
                            <div style={{ borderRadius: 10, padding: 14, background: theme.hoverBg, border: thinBorder, display: "flex", flexDirection: "column", gap: 10 }}>
                              <div style={{ borderRadius: 10, padding: 14, background: theme.cardBg, border: thinBorder }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                                  ...
                                  <span style={{ fontSize: 15, fontWeight: 500, color: "#10A37F" }}>ChatGPT (GPT-4o)</span>
                                  <MentionBadge mentioned={q.gpt} />
                                </div>
                                {fullAnswerView?.queryId === q.id && fullAnswerView?.engine === "gpt" ? (
                                  <div>
                                    <div style={{ fontSize: 15, lineHeight: 1.8, color: theme.text, whiteSpace: "pre-line" }}>{(q as any).gptFull || q.gptSnippet}</div>
                                    <HoverButton onClick={(e) => { e.stopPropagation(); setFullAnswerView(null); }} theme={theme} style={{ marginTop: 10, padding: "4px 12px", fontSize: 15, fontWeight: 500, color: "#10A37F", background: "none", border: "1px solid #10A37F", borderRadius: 9, cursor: "pointer" }}>Hide full answer</HoverButton>
                                  </div>
                                ) : (
                                  <div>
                                    <p style={{ fontSize: 15, lineHeight: 1.6, color: theme.text, margin: 0 }}>{q.gptSnippet}</p>
                                    {(q as any).gptFull && (<HoverButton onClick={(e) => { e.stopPropagation(); setFullAnswerView({ queryId: q.id, engine: "gpt" }); }} theme={theme} style={{ marginTop: 8, padding: "4px 12px", fontSize: 15, fontWeight: 500, color: "#10A37F", background: "none", border: "1px solid #10A37F", borderRadius: 9, cursor: "pointer" }}>View full answer</HoverButton>)}
                                  </div>
                                )}
                              </div>
                              <div style={{ borderRadius: 10, padding: 14, background: theme.cardBg, border: thinBorder }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                                  ...
                                  <span style={{ fontSize: 15, fontWeight: 500, color: "#727272" }}>Google Gemini</span>
                                  <MentionBadge mentioned={q.gemini} />
                                </div>
                                {fullAnswerView?.queryId === q.id && fullAnswerView?.engine === "gemini" ? (
                                  <div>
                                    <div style={{ fontSize: 15, lineHeight: 1.8, color: theme.text, whiteSpace: "pre-line" }}>{(q as any).geminiFull || q.geminiSnippet}</div>
                                    <HoverButton onClick={(e) => { e.stopPropagation(); setFullAnswerView(null); }} theme={theme} style={{ marginTop: 10, padding: "4px 12px", fontSize: 15, fontWeight: 500, color: "#727272", background: "none", border: "1px solid #727272", borderRadius: 9, cursor: "pointer" }}>Hide full answer</HoverButton>
                                  </div>
                                ) : (
                                  <div>
                                    <p style={{ fontSize: 15, lineHeight: 1.6, color: theme.text, margin: 0 }}>{q.geminiSnippet}</p>
                                    {(q as any).geminiFull && (<HoverButton onClick={(e) => { e.stopPropagation(); setFullAnswerView({ queryId: q.id, engine: "gemini" }); }} theme={theme} style={{ marginTop: 8, padding: "4px 12px", fontSize: 15, fontWeight: 500, color: "#727272", background: "none", border: "1px solid #727272", borderRadius: 9, cursor: "pointer" }}>View full answer</HoverButton>)}
                                  </div>
                                )}
                              </div>
          ...
          <div>
            <div style={{ ...card, padding: "12px 18px", marginBottom: 14 }}>
              <p style={{ fontSize: 15, color: theme.text, margin: 0 }}>
                <span style={{ fontWeight: 500 }}>SEO Keywords</span> - Terms where All4Horses ranks on Google, including AI engine connection.
                <span style={{ display: "block", fontSize: 15, color: theme.textSecondary, marginTop: 4 }}>Improving SEO rankings directly impacts AI engine presence (GEO).</span>
              </p>
            </div>
            <div style={{ ...card, overflow: "hidden" }}>
              <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" as any }}>
              <table style={{ width: "100%", fontSize: 15, borderCollapse: "collapse", minWidth: isMobile ? 700 : "auto" }}>
                <thead>
                  <tr style={{ background: theme.tableHeaderBg, borderBottom: `1px solid ${theme.border}` }}>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}>#</th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Keyword <Tooltip text="The search term being tracked" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Rank <Tooltip text="Position in Google search results" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Change <Tooltip text="Ranking change compared to previous period" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Monthly vol. <Tooltip text="Average monthly search volume for this term" /></span></th>
                    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>Difficulty <Tooltip text="Ranking difficulty level for this term (0-100)" /></span></th>
                    <th style={{ textAlign: "center", padding: "10px 14px", fontWeight: 500, color: theme.textSecondary, fontSize: 14, textTransform: "uppercase", letterSpacing: "0.5px" }}><span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><AIEngineLogo engine="gpt" size={14} /> <AIEngineLogo engine="gemini" size={14} /></span></th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderBottom: thinBorder, background: theme.tableBg }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = theme.hoverBg; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = theme.tableBg; }}>
                      ...
                      <td style={{ padding: "10px 14px" }}>
                        ...
                        ) : (<span style={{ fontSize: 15, color: theme.textMuted }}>-</span>)}
                      </td>
        ...
        {activeTab === "audiences" && (
          <div>
            <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", marginBottom: 20, flexDirection: isMobile ? "column" : "row", gap: isMobile ? 12 : 0 }}>
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 500, color: theme.text, margin: "0 0 4px", display: "flex", alignItems: "center", gap: 8 }}>Identified target audiences <Tooltip text="Target audience personas identified from query and AI response analysis. Source: AI response analysis" /></h2>
            ...
            {showPersonaForm && (
              <div style={{ ...card, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 15, fontWeight: 500, marginBottom: 6, color: theme.text }}>Role / description</label>
                    <input type="text" placeholder="Example: Physiotherapist looking for collaboration" style={{ width: "100%", padding: "8px 12px", borderRadius: 10, fontSize: 15, border: `1px solid ${theme.border}`, color: theme.text, outline: "none", background: theme.inputBg, boxSizing: "border-box" }} />
                  </div>
                  <div style={{ gridColumn: "span 2" }}>
                    <label style={{ display: "block", fontSize: 15, fontWeight: 500, marginBottom: 6, color: theme.text }}>Detailed description</label>
                    <textarea rows={3} placeholder="Describe the persona, what they're looking for, what their needs are..." style={{ width: "100%", padding: "8px 12px", borderRadius: 10, fontSize: 15, border: `1px solid ${theme.border}`, color: theme.text, outline: "none", resize: "none", background: theme.inputBg, boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: 15, fontWeight: 500, marginBottom: 6, color: theme.text }}>Tags</label>
                    <input type="text" placeholder="Age, location, field - comma separated" style={{ width: "100%", padding: "8px 12px", borderRadius: 10, fontSize: 15, border: `1px solid ${theme.border}`, color: theme.text, outline: "none", background: theme.inputBg, boxSizing: "border-box" }} />
                  </div>
                  <div style={{ display: "flex", alignItems: "flex-end" }}>
                    <HoverButton filled onClick={() => setShowPersonaForm(false)} theme={theme} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 20px", ...btnFilled, fontSize: 15, fontWeight: 600, borderRadius: 9, cursor: "pointer" }}>
                ...
                <div key={p.id} style={{ ...card, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: 16 }}>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, border: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 500, color: theme.text, flexShrink: 0 }}>{p.name.charAt(0)}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <h3 style={{ fontSize: 16, fontWeight: 500, color: theme.text, margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</h3>
                        <p style={{ fontSize: 15, color: theme.textSecondary, margin: "2px 0 0" }}>{p.role}</p>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 24, fontWeight: 500, color: theme.text }}>{p.score}%</div>
                        <div style={{ fontSize: 15, color: theme.textMuted }}>Relevance</div>
                      </div>
                    </div>
                    <p style={{ fontSize: 15, lineHeight: 1.6, color: theme.text, margin: "0 0 12px" }}>{p.description}</p>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                      {p.tags.map((tag, i) => (<span key={i} style={{ fontSize: 15, padding: "3px 10px", borderRadius: 10, border: thinBorder, background: theme.badgeBg, color: theme.text }}>{tag}</span>))}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 16, paddingTop: 12, borderTop: thinBorder }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 15, color: theme.textSecondary }}>{p.queries} queries</span>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 15, color: theme.textSecondary }}>{p.mentions} mentions</span>
                      </div>
                      <div style={{ flex: 1 }} />
                      <HoverButton onClick={() => { setPersonaFilter(p.id); setActiveTab("queries"); }} theme={theme} style={{ fontSize: 15, fontWeight: 500, color: "#10A37F", background: "transparent", border: "none", cursor: "pointer", textDecoration: "underline" }}>View queries</HoverButton>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {activeTab === "products" && (
          <div>
            <div style={{ ...card, padding: "12px 18px", marginBottom: 14 }}>
              <p style={{ fontSize: 15, color: theme.text, margin: 0 }}>
                <span style={{ fontWeight: 500 }}>GeoScale identified</span> <span style={{ fontWeight: 500 }}>5 services and 1 product</span> from scanning <span style={{ fontWeight: 500 }}>all4horses.co.il</span>.
                <span style={{ display: "block", fontSize: 15, color: theme.textSecondary, marginTop: 4 }}><strong>Services</strong> - activities the business provides to clients (riding, therapy, tours). <strong>Products</strong> - physical items for sale (gear, accessories).</span>
              </p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              {([{ key: "all" as const, label: "All", count: 6 }, { key: "service" as const, label: "Services", count: 5 }, { key: "product" as const, label: "Products", count: 1 }]).map((f) => (
                <HoverButton key={f.key} onClick={() => setProductFilter(f.key)} filled={productFilter === f.key} theme={theme} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 15, fontWeight: productFilter === f.key ? 600 : 500, ...(productFilter === f.key ? btnActive : btnInactive), cursor: "pointer" }}>
                  {f.label} ({f.count})
                </HoverButton>
              ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 500, color: theme.text, margin: "0 0 4px", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {(() => {
              ...
              const renderProductCard = (p: typeof allProducts[0], i: number) => (
                <div key={i} style={{ ...card, overflow: "hidden" }}>
                  <div style={{ height: 3, background: p.type === "Product" ? "#10A37F" : "#727272" }} />
                  <div style={{ padding: 16 }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                          <h3 style={{ fontSize: 16, fontWeight: 500, color: theme.text, margin: 0 }}>{p.name}</h3>
                          <span style={{ fontSize: 15, padding: "2px 8px", borderRadius: 10, background: theme.badgeBg, color: p.type === "Product" ? "#10A37F" : "#727272", fontWeight: 500, border: `1px solid ${theme.border}` }}>{p.type}</span>
                          <span style={{ fontSize: 15, padding: "2px 8px", borderRadius: 10, background: theme.badgeBg, color: theme.textSecondary, fontWeight: 500, border: `1px solid ${theme.border}` }}>{p.audience}</span>
                        </div>
                      </div>
                      <div style={{ width: 44, height: 44, borderRadius: 10, background: theme.hoverBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ fontSize: 20, fontWeight: 500, color: p.score >= 70 ? "#10A37F" : theme.text }}>{p.score}%</span>
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
                      <div style={{ textAlign: "center", padding: "6px 0", background: theme.hoverBg, borderRadius: 8 }}>
                        <div style={{ fontSize: 18, fontWeight: 500, color: theme.text }}>{p.queries}</div>
                        <div style={{ fontSize: 15, color: theme.textSecondary }}>Queries</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "6px 0", background: theme.hoverBg, borderRadius: 8 }}>
                        <div style={{ fontSize: 18, fontWeight: 500, color: "#10A37F" }}>{p.mentioned}</div>
                        <div style={{ fontSize: 15, color: theme.textSecondary }}>Mentioned</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "6px 0", background: theme.hoverBg, borderRadius: 8 }}>
                        <div style={{ fontSize: 18, fontWeight: 500, color: (p.queries - p.mentioned) > 3 ? "#727272" : theme.text }}>{p.queries - p.mentioned}</div>
                        <div style={{ fontSize: 15, color: theme.textSecondary }}>Missing</div>
                      </div>
                    </div>
                    <div style={{ padding: 8, background: theme.hoverBg, borderRadius: 8, border: thinBorder }}>
                      <p style={{ fontSize: 15, fontWeight: 500, color: "#10A37F", margin: "0 0 4px" }}>Top query</p>
                      <p style={{ fontSize: 15, color: theme.text, margin: 0 }}>&quot;{p.topQuery}&quot;</p>
                    </div>
                  </div>
                </div>
              );
                ...
                <>
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <h3 style={{ fontSize: 16, fontWeight: 500, color: theme.text, margin: 0 }}>Services</h3>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <h3 style={{ fontSize: 16, fontWeight: 500, color: theme.text, margin: 0 }}>Products</h3>
        ...
        {activeTab === "content" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <h2 style={{ fontSize: 20, fontWeight: 500, color: theme.text, margin: 0, display: "flex", alignItems: "center", gap: 8 }}>Content Creation <Tooltip text="GEO-Optimized article creation queue for selected queries" /></h2>
                <span style={{ fontSize: 15, padding: "2px 10px", borderRadius: 10, background: theme.badgeBg, color: theme.textSecondary, border: thinBorder }}>{contentQueue.length} items</span>
              </div>
              <HoverButton filled onClick={() => setActiveTab("queries")} theme={theme} style={{ padding: "6px 14px", fontSize: 15, fontWeight: 600, ...btnFilled, borderRadius: 8, cursor: "pointer" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ ...card, padding: "12px 18px", marginBottom: 16, borderLeft: "3px solid #10A37F" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <p style={{ fontSize: 15, color: theme.text, margin: 0 }}><span style={{ fontWeight: 500 }}>GEO-Optimized format</span> - Content is built to appear in AI engine answers. Each article is optimized for a specific query.</p>
              </div>
            </div>
            {contentQueue.length === 0 ? (
              <div style={{ ...card, padding: 48, textAlign: "center" }}>
                ...
                <p style={{ fontSize: 15, fontWeight: 500, color: theme.text, margin: "0 0 6px" }}>No content in queue</p>
                <p style={{ fontSize: 15, color: theme.textSecondary, margin: "0 0 16px" }}>Go to the <strong>Queries</strong> tab, expand a query and click <strong>&quot;Create content&quot;</strong> to get started.</p>
                <HoverButton onClick={() => setActiveTab("queries")} theme={theme} style={{ padding: "8px 20px", fontSize: 15, fontWeight: 600, color: "#10A37F", background: "transparent", border: "1px solid #10A37F", borderRadius: 8, cursor: "pointer" }}>Go to queries</HoverButton>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {contentQueue.map((qId) => {
                  ...
                  return (
                    <div key={qId} style={{ ...card, overflow: "hidden" }}>
                      <div style={{ height: 3, background: "linear-gradient(90deg, #10A37F, #727272)" }} />
                      <div style={{ padding: 16 }}>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                          <div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                              <h3 style={{ fontSize: 16, fontWeight: 500, color: theme.text, margin: 0 }}>{q.text}</h3>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 8, marginBottom: 12 }}>
                          {[{ label: "Word count", value: "1,500" }, { label: "Format", value: "GEO-Optimized" }, { label: "Language", value: "English" }, { label: "Status", value: "Pending creation" }].map((s, i) => (
                            <div key={i} style={{ textAlign: "center", padding: "8px 0", background: theme.hoverBg, borderRadius: 8 }}>
                              ...
                              <div style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>{s.value}</div>
                            </div>
                          ))}
                        </div>
                        <div style={{ padding: "10px 14px", background: theme.hoverBg, borderRadius: 8, border: thinBorder }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>Preview - AI answer</span>
                          </div>
                          <p style={{ fontSize: 15, lineHeight: 1.6, color: theme.textSecondary, margin: 0 }}>{q.gptSnippet}</p>
                        </div>
                      </div>
                    </div>
                  );
