...
const phases: Phase[] = [
  {
    ...
    color: "#10A37F",
    features: [
      // ====== YEDIOTH AHRONOTH MEETING - ALEXEI 2026-04-26 ======
      // These items are TOP priority for the Thursday meeting with Yedioth Ahronoth.
      // Reference implementation: /scale-publish (toggle Agency / Publisher view).
      { name: "[YNET] Two-User Toggle on /scale-publish (Agency / Publisher)", description: "Top of /scale-publish has a clear visual toggle: 'Agency View' (green) vs 'Publisher View' (amber). Header shows who you're logged in as (e.g. 'Just In Time Agency · Acting for Bank Hapoalim' OR 'Yedioth Ahronoth Group · Publisher'). State persists across reload. This is the centerpiece Alexei demonstrates at the Yedioth meeting - Agency sees one world, Publisher sees the mirror, both connected by shared state", ref: "Demo: /scale-publish -> top user-mode switcher | Alexei call 2026-04-26 (00:47-05:08): 'I need a user that's the agency... I need a publisher interface where the publisher inserts their sites'", priority: "P0", status: "not_started" },
      { name: "[YNET] Publisher Dashboard - Sites & Sections Manager", description: "When viewing as Publisher (Yedioth), display all 8 Yedioth properties (Ynet, Calcalist, Sport5, Mynet, Pnai Plus, Vi.B, Lady, Ynet Business) as expandable rows. Each property shows its sections (e.g. Ynet -> Cars/רכב, Sports/ספורט, Health/בריאות, Tech/דיגיטל, etc). Each section row shows: English name + Hebrew name, category pill, audience tags, monthly readership, ~3-day upload time, editable price. Add Site / Add Section buttons. Publisher must be able to manage inventory dynamically - this is the #1 ask from Yedioth ('they need to feel they can change anything anytime')", ref: "Demo: /scale-publish -> Publisher View -> Sites & Sections | Alexei call 2026-04-26 (03:00-05:31): 'every site he can edit the price... section, section, sections, section and topic. Section: Cars'", priority: "P0", status: "not_started" },
      { name: "[YNET] Publisher Dashboard - Editable Per-Section Pricing (Live)", description: "Click any price (₪3K-₪10K range) to open inline edit. Save propagates to localStorage and ANY agency viewing matching sites sees the new price immediately. Visual cue when price was overridden from default. This 'live price editing' is the dynamic feel Yedioth demanded - they need to know they can adjust pricing without engineering. Currency MUST be ₪ (NIS), never $", ref: "Demo: /scale-publish -> Publisher View -> click any section price | Alexei call 2026-04-26 (05:35-05:58): 'shi-shi-it should be dynamic, easy for them to change prices, change/delete sites, add sites, add sections'", priority: "P0", status: "not_started" },
      { name: "[YNET] Publisher Dashboard - Order Inbox (Pending Orders)", description: "Inbox tab in Publisher view lists all incoming orders from agencies, filterable by status (pending / approved / in progress / published / rejected). Each order card shows: agency name, brand, # queries, # sections, total ₪, badge for status. Click to expand: full query list, section breakdown with prices, content mode (empty/generate), agency contact. Buttons: Approve & contact agency, Reject, Mark in progress, Mark published. Pre-seed at least 1 demo order so the inbox is never empty for the meeting", ref: "Demo: /scale-publish -> Publisher View -> Inbox tab | Alexei call 2026-04-26 (52:44-58:13): 'user publisher receives email/notification, sales rep contacts agency to collect payment, then approves the article'", priority: "P0", status: "not_started" },
      { name: "[YNET] Publisher Dashboard - Article Tracking (Per-Item Monitoring)", description: "Articles & Tracking tab lists every published article (item-level, NOT domain-level). Per article: URL, parent site/section, publish date, view count, crawl status, Google indexed Y/N, GPT/Gemini/Perplexity citation Y/N, impact score 0-100. Expandable row reveals 'Queries this article ranks for' table (query, engine, rank). Action buttons: Open article, Re-check engines, Export report. This is the retention hook - Yedioth's agencies will keep buying because they can prove ROI", ref: "Demo: /scale-publish -> Publisher View -> Articles & Tracking | Alexei call 2026-04-26 (06:48-07:46, 51:33-52:02): 'tracking after them. Were they caught, scanned... it's not domain monitoring, it's items. Each article is an item'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Multi-Select Queries (Max 5 + Dismiss + Pin)", description: "Agency view's My Queries tab. Each query row has a checkbox (cap at 5 selections), an X to dismiss, and a bookmark pin. Selection counter at the top: 'X of 5 selected'. Disabled state when 5 already selected. Restore-all link for dismissed queries. This is the entry point for the entire ordering flow - per Alexei the agency starts here every time", ref: "Demo: /scale-publish -> Agency View -> My Queries | Alexei call 2026-04-26 (13:36-15:06, 25:14): 'add option in UX where when he chooses queries when doing external article that he can choose up to 5 queries... do select... once selected, the entire pool become...'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Order Flow with Matching Yedioth Sections (LIST not boxes, priority-sorted)", description: "After selecting queries -> 'Continue to content creation' button -> Order Flow tab shows: Step 1 selected query chips, Step 2 editable title (auto-suggested), Step 3 content mode (AI-generate vs empty), Step 4 a LIST of Yedioth sections sorted by fit score high->low. CRITICAL: must be a LIST not boxes/cards (Alexei explicitly rejected boxes because priority order requires linear scanning). Each row: rank #, favicon, site name, section name (English + Hebrew), match reason pills (audience/category/query), monthly readership, ~3 day upload, fit score 0-100, price ₪, Add button", ref: "Demo: /scale-publish -> Agency View -> Order Flow | Alexei call 2026-04-26 (35:34): 'Nir, this should be a LIST and NOT squares because it should be by priority from highest to lowest, so it's not logical to put squares'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Match Reasoning Pills (Audience / Category / Query Intent)", description: "Each matched section row shows WHY it was matched, as colored pills: green 'Audience: B2C + Decision-makers' (audience overlap), blue 'Category: Finance' (query category match), amber 'X/5 queries match intent' (query-intent overlap). User must be able to read at a glance why this site is recommended. Match algorithm joins selected queries' audience tags, categories, intent against each section's metadata", ref: "Demo: /scale-publish -> Agency View -> Order Flow -> match reason pills on each row | Alexei call 2026-04-26 (15:34-15:53): 'site that suits this query for this audience... by category, query, audience'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Editable Title Triggers Re-Match", description: "Article title field above the matching list. Auto-suggested from first selected query (e.g. 'Best banking app in Israel 2026 — Bank Hapoalim's complete 2026 guide'). When user edits, the matching algorithm re-runs and the section list re-orders. Hint text: 'Editing the title re-evaluates which Yedioth sections are the best fit'. Hebrew + English supported. Per Alexei this is critical - manual title overrides are common and the system must adapt", ref: "Demo: /scale-publish -> Agency View -> Order Flow -> Step 2 title field | Alexei call 2026-04-26 (27:05-27:46): 'edit the headline, write here in Hebrew or English, and if he changes the headline manually we automatically need to know to match the more correct sites'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Content Mode Choice (AI-Generate vs Empty)", description: "Step 3 of Order Flow: two large radio cards - 'AI-generated draft' (we generate from queries, Yedioth editor reviews) vs 'I'll provide the copy' (agency/client writes, we just place). This matches Yedioth's reality - sometimes Bank Hapoalim wants to control wording, other times they want speed. Default selection: AI-generate", ref: "Demo: /scale-publish -> Agency View -> Order Flow -> Step 3 | Alexei call 2026-04-26 (35:48): 'either you write the article empty/blank or you generate the article'", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency Submit Order -> Publisher Inbox (Cross-State)", description: "When agency clicks 'Submit order to Yedioth', a new Order is appended to localStorage with status=pending. Publisher view's Inbox immediately shows the new order with red notification badge. Toggle back to Agency -> the order shows in My Orders with same ID. This live cross-state demo is the #1 wow moment for the Yedioth meeting - shows the marketplace is real and connected. Confirmation screen shows '~3 day publication window'", ref: "Demo: /scale-publish -> Agency submit -> toggle to Publisher -> see new order | Alexei call 2026-04-26 (37:40-58:13): the entire money-flow narrative he wants to demonstrate", priority: "P0", status: "not_started" },
      { name: "[YNET] Agency - Per-Article Tracking Drilldown (with Export Report)", description: "Article Tracking tab in Agency view. Same data structure as Publisher tracking, framed for the agency: 'Per-Article Tracking · X published articles · Y queries currently ranking'. KPI strip: Total views, Google-indexed, AI-cited, Ranking queries. Each article row expandable to show ranked queries table. Closing pitch text at bottom: 'Today, agencies buy articles from publishers without ever knowing if they got picked up. Geoscale ScalePublish monitors every item - if Bank Hapoalim's article on Ynet gets cited by ChatGPT, you can prove it'. Export Report button (mock for demo)", ref: "Demo: /scale-publish -> Agency View -> Article Tracking | Alexei call 2026-04-26 (07:30-07:46): 'they can pull a report from the regular agency of what was caught and what wasn't from the articles, because that doesn't even exist now in the agency'", priority: "P0", status: "not_started" },
      { name: "[YNET] Pre-Seed Demo Data (Yedioth + Bank Hapoalim Brand Scan)", description: "Pre-load demo data so meeting opens to a populated state, never empty: 8 Yedioth sites + 30 sections with realistic ₪ prices and Hebrew names; 12 Bank Hapoalim brand-scan queries with category/audience/opportunity score; 1 pending order in Publisher inbox (TechStart Israel ordering on Calcalist); 2 historical published articles with full tracking metadata (one cited by GPT+Gemini, ranking #1-#3 for multiple queries). Alexei should NEVER demo on an empty UI", ref: "Demo: /scale-publish first load shows pre-populated data | Alexei call 2026-04-26 (40:15): 'Nir and Noam will populate manually all sites' - we did this in code so meeting is ready", priority: "P0", status: "not_started" },
      // ====== END YEDIOTH MEETING SECTION ======
      ...
      // Citation Sources UX upgrades - 2026-04-25 (Nir)
      { name: "Citation Sources - Insight Banner (Auto Gap Detection)", description: "When user views a competitor tab, surface the single biggest opportunity at the top: e.g. 'horseforum.com cites Galilee Horses 18x but you 0 - biggest opportunity to close the gap', with an inline 'Order placement' CTA. Auto-computed by joining competitor citations against the brand's citation map and ranking by gap (theirs - mine)", ref: "Demo: /scan -> Citation Sources -> select competitor tab -> amber banner | Production: missing - prevents user from having to scan the table to find action items", priority: "P0", status: "not_started" },
      { name: "Citation Sources - Search + Type Filter Pills", description: "Above the table: domain search input + type filter pills (All / Publication / Social / News / Blog / Review / Forum) so user can narrow by content type ('show me only Forum sources where competitors get cited but I don't'). Empty state has a Clear filters button", ref: "Demo: /scan -> Citation Sources filter row | Production: missing - tables become unusable past 10 rows without filtering", priority: "P0", status: "not_started" },
      { name: "Citation Sources - 'vs You' Comparison Column", description: "When viewing a competitor tab, add a 'vs you' column with a colored diff badge: amber '+8 gap' if competitor cites X more than you, green '-3 ahead' if you cite more, neutral 'same'. Sortable to surface biggest opportunities first. Hidden when viewing your own brand tab", ref: "Demo: /scan -> Citation Sources -> competitor tab -> 'vs you' column | Production: missing - turns the table from passive data into actionable competitive intel", priority: "P0", status: "not_started" },
      { name: "Citation Sources - Expandable Rows with URL Drilldown", description: "Click a row to expand and reveal the actual cited URLs from that source (chevron rotates 90deg). Each URL is clickable. Inside the expanded panel: 'Order placement on {domain}' green CTA + 'Visit site' secondary button. Mock URLs generated per source-kind (Publication/Forum/Social/etc) for realism in demo", ref: "Demo: /scan -> Citation Sources -> click row to expand | Production: missing - flat table with no way to drill in", priority: "P0", status: "not_started" },
      { name: "Citation Sources - Tab Badges with URL Totals", description: "Each tab in the Citation Sources tab bar shows a count badge with total URLs cited for that brand/competitor (e.g. 'Galilee Horses [45]'). Active tab badge is green-filled; inactive tabs use muted background. Lets user see scale of each entity at a glance without switching tabs", ref: "Demo: /scan -> Citation Sources tab bar | Production: tabs are text-only, no scale signal", priority: "P0", status: "not_started" },
      { name: "Citation Sources - Sortable Column Headers", description: "Clickable th on URLs cited and 'vs you' columns. Active column header has full text color + arrow indicator; inactive is muted. Resetting tabs also resets sort state to default (URLs cited desc)", ref: "Demo: /scan -> Citation Sources column headers | Production: missing", priority: "P0", status: "not_started" },
    ],
  },
  {
    id: 2,
